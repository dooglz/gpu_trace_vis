class Task {
    constructor(start = 0, end = 0, cat = 0, name = "", type = -1 ) {
        console.log(type);
...
class TaskChart {
    constructor(container, data, categoryCount) {
    ...
    Redraw() {
        ...
        var rectTransform = function (d) {
            return "translate(" + this.xScale(d.start) + this.padding[2] + "," + this.yScale(d.cat) + ")";
        };

        this.svg.append("g").selectAll('.task').data(this.data).enter()
            .append("rect")
            .attr("rx", 5)
            .attr("ry", 5)
    ...
    InitDomains() {
        ...
        this.max = this.data.reduce(
            ...
            , this.data[0].end);
            console.log(this.min, this.max, this.width - (this.padding[2] + this.padding[3]));
        this.xScale = d3.scale.linear().domain([this.min, this.max]).range([0, this.width - (this.padding[2] + this.padding[3])]).clamp(true);
        this.yScale = d3.scale.ordinal().domain(d3.range(this.categoryCount)).rangeRoundBands([0, this.height - (this.padding[0] + this.padding[1])]);
    }
}